    ...
    $.fn.jsonEdit = function (options) {
        ...
        function valueChanged(changedObject) {
            ...
            if (changedObject.prop("tagName").toLowerCase() == "input" && changedObject.prop("type").toLowerCase() == "checkbox") {
                if(changedObject.attr("elementtype") != undefined && changedObject.attr("elementtype").toLowerCase() == "boolean"){
                    p = p + "=" + (changedObject.prop("checked") == true ? "true" : "false") + ";";
                }else{
                    var buffer_array_selecionado = [];

                    var selecionados = $("input[name^='"+ changedObject.attr("name") +"']:checked:enabled");
                    selecionados.each(function( index ) {
                        buffer_array_selecionado.push( $( this ).val() );
                    });
                    p = p + "='"  +  JSON.stringify(buffer_array_selecionado)    + "';";
                    //var buffer_string_values = "";
                    //for(var i = 0; i < buffer_array_selecionado.length; i++){
                    //    buffer_string_values += buffer_array_selecionado[i];
                    //    if(i < buffer_array_selecionado.length - 1){
                    //        buffer_string_values += ",";
                    //    }
                    //}
                    //p = p + "='"  +  buffer_string_values    + "';";
                }
            } else {
            ...
            if (options["afterValueChanged"]) options["afterValueChanged"](options["value"], options["schema"], JSON.parse(changedObject.attr("data-path").replaceAll("'", "\"")));
        }
        ...
        function renderSimpleNode(schemaNode, schemaName, isRequired) {
            ...
            if (nodeType == "boolean") {
                ...
                inputBody = '<input elementtype="boolean" type="checkbox" ' + classAtt + dataValueNameAtt + requiredAtt + ' />';
            } else {
                if (fixNU(schemaNode["enum"], "") == "") {
                    ...
                    if (editor == "checkbox") {
                        ...
                        jQuery.each(schemaNode["enum"], function (index) {
                            inputBody += '<label><input elementtype="select" value="' + schemaNode["enum"][index] + '" type="checkbox" ' + classAtt + nameAtt + dataValueNameAtt + requiredAtt
                                + ' /> <span class="j-input-radio-label">' + schemaNode["enum"][index] + '</span></label>&nbsp;&nbsp;&nbsp;';
                        });
                    }
        ...
        function toGambsonToArrayJson(j){
            if(typeof(j) != typeof("")) return j;
            if(j == undefined || j == "" ) return [];

            try{
                return JSON.parse(j);
            } catch(e){
                //console.error("O valor:", j, "não pode ser convertido por GanbSon");
                if( j.indexOf(",") >=0 ){
                    return j.split(",");
                } else {
                    return [j];
        ...
        function setValue(v) {
            ...
            renderPlace.find("input[data-path],select[data-path],textarea[data-path]").each(function () {
                if ($(this).prop("tagName").toLowerCase() == "input" && $(this).prop("type").toLowerCase() == "checkbox") {
                    if($(this).attr("elementtype") != undefined ) {
                        if($(this).attr("elementtype").toLowerCase() == "boolean") {
                            $(this).prop("checked", V(v, $(this).attr("data-path")) == true ? true : false);
                        } else {
                            
                            if(v[JSON.parse($(this).attr("data-path").replaceAll("'", "\""))[0]] == ""){
                                return;
                            }
                            
                            //var buffer_array_valores = [];
                            //if(v[JSON.parse($(this).attr("data-path").replaceAll("'", "\""))[0]].indexOf(",") >= 0){
                            //    buffer_array_valores = v[JSON.parse($(this).attr("data-path").replaceAll("'", "\""))[0]].split(",");
                            //}

                            var buffer_array_valores = toGambsonToArrayJson(v[JSON.parse($(this).attr("data-path").replaceAll("'", "\""))[0]]);
                            //console.log("Path:", buffer_array_valores);
                            if(buffer_array_valores != undefined) {
                                for(var ji = 0; ji < buffer_array_valores.length; ji++){
                                    
                                    if(typeof(buffer_array_valores[ji]) == typeof("")) {
                                        if(buffer_array_valores[ji].trim() == "") continue;
                                        $('[data-path="' + $(this).attr("data-path") + '"][value="'+ buffer_array_valores[ji] +'"]').prop("checked", true);
                                    } else {
                                        var first_key = Object.keys(buffer_array_valores[ji])[0];
                                        $('[data-path="' + $(this).attr("data-path") + '"][value="'+ buffer_array_valores[ji][first_key] +'"]').prop("checked", true);
                                    }
                                }
                            }
                        }
